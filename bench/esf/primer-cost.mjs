...
const { encode: esfEncode } = require("../../esf");

// ESF's token win over columnar JSON is only real AFTER you pay to teach the model the
...
// break-even message count where ESF's per-message savings overcome its primer overhead.
...
const primers = {
  ...
  ESF:
    "Messages use ESF, a compact text format. The first line is the magic header !esf/1. " +
...
// Per-message savings come from the real corpus (the 5 handoff docs), ESF vs columnar JSON.
...
const pExtra = primerTok.ESF - primerTok.columnar; // extra one-time tokens ESF needs vs columnar

console.log("# Primer cost & ESF-vs-columnar break-even\n");
...
console.log(`\nESF needs **${pExtra}** more primer tokens than columnar JSON (one-time, cacheable).\n`);

console.log("## Per-message savings: ESF vs columnar JSON\n");
console.log("| Message shape | ESF tok | columnar tok | ESF saves |");
console.log("|---|---:|---:|---:|");
let totalEsf = 0, totalCol = 0;
for (const [name, d] of Object.entries(datasets)) {
  const e = o200kTokens(esfEncode(d));
  const c = o200kTokens(JSON.stringify(colEncode(d)));
  totalEsf += e; totalCol += c;
  console.log(`| ${name} | ${e} | ${c} | ${c - e > 0 ? "+" : ""}${c - e} |`);
}
const avgSave = (totalCol - totalEsf) / Object.keys(datasets).length;
console.log(`\nAverage saving **${avgSave.toFixed(0)} tok/message** (range shows it is shape-dependent — scalar envelopes can be negative).\n`);

console.log("## Break-even (how many messages before ESF nets ahead of columnar)\n");
...
console.log(`a cached agent pipeline recovers ESF's primer in a handful of messages; a low-volume`);
