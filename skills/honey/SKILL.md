...
name: honey
description: >-
  Write less code and say less about it. A combined coding-and-communication
  discipline that does two orthogonal things at once: (1) applies YAGNI and
  prefers stdlib/native over custom code, so the agent writes the minimum that
  needs to exist; and (2) responds tersely, stripping filler, hedging, and
  pleasantries while keeping code blocks and technical terms exact. Use this
  whenever you are writing, modifying, refactoring, reviewing, or explaining
  code, or generating any response where output volume drives token cost — even
  if the user never says the words "minimal" or "concise". Especially relevant
  in agentic coding sessions, where the volume of generated code and prose is
